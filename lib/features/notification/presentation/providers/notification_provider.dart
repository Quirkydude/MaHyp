...
class NotificationNotifier extends StateNotifier<NotificationState> {
  ...
  void _initNotifications() {
    ...
    _ref.listen<AsyncValue<List<MedicationModel>>>(medicationProvider, (
      previous,
      next,
    ) {
  ...
  void _loadInitialNotifications() {
    _loadFromFirestore()
        .then((persisted) {
          ...
          medications.whenData(
            (meds) => _generateMedicationNotifications(meds),
          );
        ...
        if (dose.currentStatus == MedicationStatus.missed) {
          newNotifications.add(
            NotificationItem(
              ...
              body:
                  'You missed your ${med.dosage} dose. Please take it as soon as possible if safe.',
              type: NotificationType.medicationReminder,
  ...
  void _generateBPNotifications(List<BPReadingModel> readings) {
      ...
      if (category == BPCategory.crisis) {
        newNotifications.add(
          NotificationItem(
            ...
            body:
                'Your reading of ${latest.systolic}/${latest.diastolic} mmHg is dangerously high. Seek emergency medical attention immediately.',
        ...
        newNotifications.add(
          NotificationItem(
            ...
            body:
                'Your reading of ${latest.systolic}/${latest.diastolic} mmHg is not controlled (140/90 or higher). Take your medication and contact your doctor if readings remain high.',
            type: NotificationType.bpReminder,
            createdAt: latest.recordedAt,
            isRead: false,
            payload: {
        ...
        newNotifications.add(
          NotificationItem(
            ...
            body:
                'Your blood pressure of ${latest.systolic}/${latest.diastolic} mmHg is in the normal range. Keep it up!',
            type: NotificationType.system,
            createdAt: latest.recordedAt,
    ...
    final hasReadingToday = readings.any(
      (r) => r.recordedAt.isAfter(todayStart),
    );

    if (!hasReadingToday && now.hour >= 9) {
      // Only remind after 9 AM
      newNotifications.add(
        NotificationItem(
          id: 'bp_reminder_${now.year}_${now.month}_${now.day}',
          title: 'BP Check Reminder',
          body:
              "You haven't measured your blood pressure today. Regular monitoring helps track your health.",
          type: NotificationType.bpReminder,
  ...
  void _mergeNotifications(List<NotificationItem> newNotifications) {
    ...
    final existingNotifications = List<NotificationItem>.from(
      state.notifications,
    );
  ...
  void deleteNotification(String id) {
    ...
    col
        ?.doc(id)
        .delete()
        .catchError((e) => debugPrint('Delete notif error: $e'));
    _saveToFirestore(updated);
  }
  ...
  void clearAll() {
    ...
    for (final n in state.notifications) {
      col
          ?.doc(n.id)
          .delete()
          .catchError((e) => debugPrint('Clear notif error: $e'));
    }
